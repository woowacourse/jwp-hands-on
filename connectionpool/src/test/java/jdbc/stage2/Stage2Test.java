...
class Stage2Test {
     ...
     * 하지만 **DataSource를 여러 개 사용하거나 세부 설정을 하려면 빈을 직접 생성**하는 방법을 사용한다.
     * DataSourceConfig 클래스를 찾아서 어떻게 빈으로 직접 생성하는지 확인해보자.
     * 그리고 아래 DataSource가 직접 생성한 빈으로 주입 받았는지 getPoolName() 메서드로 확인해보자.
     */

    // 빈으로 DataSource 객체를 생성하여 등록한 경우에만 주입됨!
    // 스프링 부트 설정시 생성되는 DataSource와는 별개
    @Autowired
    ...
    void test() throws InterruptedException {
        ...
        final var hikariPool = getPool(hikariDataSource);
    ...
    // 학습 테스트를 위해 HikariPool을 추출 (private 필드 'pool'에 존재)
